...
/**
 * exports.addHallsToATheater = async (req, res) => {
//validation of tha savedTheatre will be done in the later section as
middleware
const savedTheatre = await Theatre.findOne({ _id: req.params.id });
//Validation of these hall ids will be done in the later section
hallIds = req.body.hallIds;
//Add hallIds to the theatres
if (req.body.insert) {
hallIds.forEach(hallId => {
savedTheatre.halls.push(hallId);
});
} else {
//remove these halls from the theatres
savedHallIds = savedTheatre.halls;
hallIds.forEach(hallId => {
savedHallIds = savedHallIds.filter(smi => smi != hallId);
});
savedTheatre.halls = savedHallIds;
}
